...
func (ordStr *OrderStore) GetOrderItemsByOrderID(ctx context.Context, orderID int, role string, userID int) ([]dto.CartItem, error) {
	if role == "customer" {
		...
		rows, err := ordStr.BaseRepsitory.DB.Query(query, userID, orderID)
		if err != nil {
			log.Println("error in customer get orderitems by id" + err.Error())
		...
		log.Printf("in admin: order item by order id: %v", orderID)
		query := `
		...
		rows, err := ordStr.BaseRepsitory.DB.Query(query, orderID)
		if err != nil {
			log.Println("error in customer get orderitems by id" + err.Error())
			return []dto.CartItem{}, err
		}
